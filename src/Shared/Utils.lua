...
function Utils.TimeFormat(seconds: number): string
	...
	return string.format("%02i", minutes) .. ":" .. string.format("%02i", newSeconds)
end

function Utils.LargeNumber(num)
    if not num then return end
    local newNum = num/1000
    if newNum >= 1000 then
        newNum = newNum/1000
        return math.floor(newNum) .. "M"
    end
    return math.floor(newNum) .. "K"
end

function Utils.CommaFormat(amount)
