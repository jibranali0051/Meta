--[[
    ViewportModelUtil.lua
    Author(s): Jibran

    Description: For placement and alignment of models inside viewport
]]
...
local ViewportModelUtil = {}
ViewportModelUtil.__index = ViewportModelUtil
ViewportModelUtil.ClassName = "ViewportModelUtil"
...
function ViewportModelUtil.new(vpf, camera)
	local self = setmetatable({}, ViewportModelUtil)
...
function ViewportModelUtil:SetModel(model)
	self.Model = model
...
function ViewportModelUtil:Calibrate()
	local viewport = {}
...
function ViewportModelUtil:GetFitDistance(focusPosition)
...
function ViewportModelUtil:GetMinimumFitCFrame(orientation)
